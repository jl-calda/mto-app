  ...
  return (
    <div className="mx-auto max-w-[1400px] px-5 pt-[18px]">
      ...
      <div className="overflow-hidden rounded-md border border-line bg-panel">
        <div className="grid grid-cols-[28px_minmax(0,1fr)_90px_70px_110px_90px_minmax(0,140px)] items-center gap-2.5 border-b border-line bg-panel-2 px-3.5 py-2">
        ...
        {rows.map((it) => {
          ...
          return (
            <div key={it.id} className="grid grid-cols-[28px_minmax(0,1fr)_90px_70px_110px_90px_minmax(0,140px)] items-center gap-2.5 border-b border-line px-3.5 py-2 last:border-b-0">
              ...
              <div className="mono text-[12px]">{it.length ? `${it.length.toLocaleString()}mm` : '—'}</div>
              <div className="mono text-[12px]">{it.quantity ?? 1}</div>
              <div>
                <span className="tag" style={{ color: it.origin.kind === 'offcut' ? 'var(--annotation)' : 'var(--ink-2)' }}>{it.origin.kind}</span>
              </div>
              <div><span className="tag" style={{ color: tone.c, background: tone.b }}>{it.status}</span></div>
